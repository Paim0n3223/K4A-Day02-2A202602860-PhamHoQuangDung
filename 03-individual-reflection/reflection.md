...
- Nhóm: A1
- Candidate problem nhóm chọn: Phát hiện và hỗ trợ sửa ID switch giữa các frame — người làm multi-object tracking phải xem và đối chiếu hàng nghìn frame để tìm vị trí xảy ra ID switch trước khi sửa annotation (baseline tự báo cáo ~50 phút một vòng QA, cần bấm giờ lại trong pilot)
...
| Validation / research | Không phải người đi tìm nguồn (phần này do Ngô Gia Quốc và Nguyễn Hải Đăng phụ trách). Việc tôi làm là đọc lại các link trong mục 4.2 và kiểm tra mức độ uy tín của nguồn — xem đây có phải bài báo, repo hoặc tài liệu chính thức hay không, thay vì nhận số liệu mà không verify. | Bảng research cuối có 9 nguồn, đều có link gốc kiểm được: 4 bài báo (survey MOT trên tạp chí Electronics, systematic review MOT trong chăn nuôi, user study về tagging video camera trap, paper gốc ByteTrack) và 5 repo/tài liệu chính thức (SlowFast, CLIP-ReID, Ultralytics, CVAT, TrackEval). Điều này giúp nhóm giữ đúng nguyên tắc của worksheet là không dùng số liệu AI đưa nếu không verify được link chính thức. |
| Workflow nhóm | Đề xuất phiên bản đầu tiên của workflow cho bài ID switch, trước khi nhóm chỉnh sửa và bổ sung thành bản 8 bước như trong mục 5.1 hiện tại. | Bản đầu của tôi là điểm khởi đầu để nhóm bàn tiếp; bản cuối giữ mạch "chạy tracker → xem tuần tự → đối chiếu → xác nhận frame switch → sửa → lưu", bổ sung thời gian từng bước, tách riêng bước 3 là sự kiện giao nhau/che khuất làm identity bị gán nhầm, và thêm bước 8 kiểm tra lại sau khi sửa. |
...
```text
Dấu tay rõ nhất là phiên bản đầu tiên của workflow bài ID switch do tôi đề xuất — bản 8 bước ở mục
...
Nếu làm lại, tôi sẽ challenge nhóm mạnh hơn ở Phase 4. Bảng validation hiện chưa có interview hay
survey trực tiếp nào, chỉ có self-report của một thành viên và một nghiên cứu công bố làm bằng chứng
thứ cấp; nhóm có ghi trung thực chỗ này nhưng
...
- [x] [15đ] Nhóm có workflow trước/sau — mục 5.1 (8 bước, ~50 phút) và 5.2 (future + fallback)
