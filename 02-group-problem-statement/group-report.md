...
|---|---:|---|---|---|
| Interview | 0 | **Chưa thực hiện**, vì vậy chưa có quote phỏng vấn độc lập và không tính dòng này là evidence. | Chưa biết pain có lặp lại với annotator/AI Engineer khác hay chỉ đặc thù một dự án. | Giữ trạng thái validation là “Not Yet”; cần phỏng vấn ít nhất 2 người khác trước khi Go. |
| Survey / poll | 0 | **Chưa thực hiện**, chưa có dữ liệu để kết luận tần suất hoặc thời gian trung vị. | Các con số hiện tại có thể bị lệch do tự nhớ lại. | Pilot phải bấm giờ trên nhiều video và ghi log thay vì dùng ước lượng. |
| Review problem card + case dự án hiện có | 1 thành viên, 1 dự án tracking hành vi lợn; video mẫu khoảng 1.800 frame | Quote nguyên văn từ individual scan: “mỗi lần switch id 1 con lợn thì nó sẽ kéo dài cái lỗi switch đó ở các frame tiếp theo, việc check và sửa chữa có thể mất tới vài ngày”. Card chi tiết ghi một vòng hiện tại khoảng 50 phút và mục tiêu 15 phút. | Đây là self-report, chưa có timestamp, số ID switch thật hay ground truth để kiểm chứng; “tới vài ngày” không phải số trung vị. | Thu hẹp từ “AI tự phát hiện và sửa” thành “hệ thống xếp hạng các cửa sổ frame nghi vấn; người xác nhận và sửa”. Baseline 50 phút chỉ là số tạm dùng để thiết kế pilot. |

**Insight sau validation (1-2 câu — pain thật nằm ở đâu):**

```text
Tín hiệu hiện có cho thấy pain lớn nhất không nằm ở thao tác đổi nhãn mà ở việc xem tuần tự hàng nghìn frame để tìm frame bắt đầu đổi ID, đặc biệt sau giao nhau hoặc che khuất. Tuy nhiên bằng chứng mới là một case tự báo cáo, nên nhóm chưa được phép khái quát cho nhiều người hoặc cam kết mức giảm 70% trước khi đo pilot.
```

Bằng chứng hiện có: [`../01-individual-problem-scan/individual-report (2).md`](../01-individual-problem-scan/individual-report%20(2).md). Chưa có file survey/interview độc lập.

### 4.2. Research giải pháp đã có (ít nhất 2-3 tools/patterns + 1-2 link kiểm được)

| Nguồn / tool / case | Link | Họ giải quyết bước nào? | Điểm mạnh | Khoảng trống / rủi ro | Bài học cho nhóm |
|---|---|---|---|---|---|
| TrackEval — bộ đánh giá MOT | https://github.com/JonathonLuiten/TrackEval | So sánh tracker output với ground truth và tính HOTA, CLEAR, Identity; code CLEAR có trường `IDSW`. | Là baseline chuẩn, chạy được trên dữ liệu định dạng MOTChallenge và cho metric định lượng thay vì chỉ xem video. | Cần ground truth; đây là công cụ **đánh giá**, không tự tìm/sửa annotation sai khi ground truth chưa đáng tin. | Dùng IDSW/IDF1 làm metric kết quả và tạo nhãn pilot, không gọi TrackEval là giải pháp tự sửa. |
| CVAT Track mode + AI/OpenCV trackers | https://docs.cvat.ai/docs/manual/basics/track-mode-basics/ ; https://docs.cvat.ai/docs/annotation/tools/ai-tools/ | Tạo/chỉnh track qua video, nội suy giữa keyframe và hỗ trợ tracker bán tự động. | Có giao diện để người review đi tới frame nghi vấn, kiểm tra và sửa track; phù hợp làm human boundary. | Nội suy/tracker vẫn có thể drift hoặc đổi identity khi che khuất; tài liệu không cho thấy cơ chế tự động xác nhận mọi ID switch. | Không xây lại annotation UI; xuất danh sách frame/window nghi vấn để người dùng sửa trong CVAT. |
| Ultralytics Multi-Object Tracking (BoT-SORT/ByteTrack và ReID) | https://docs.ultralytics.com/modes/track/ | Chạy tracking và duy trì ID qua frame; cho phép đổi tracker/cấu hình và dùng đặc trưng appearance/ReID ở một số tracker. | Tạo baseline nhanh, xuất ID/bounding box và thử các cấu hình association mà không phải huấn luyện hệ thống end-to-end mới. | Đổi tracker có thể giảm một số switch nhưng không bảo đảm phát hiện/sửa lỗi trong annotation sẵn có; kết quả phụ thuộc detector, cảnh và ngưỡng. | So sánh rule/ByteTrack với phương án có appearance/ReID; chỉ đưa case confidence thấp cho người review. |

**Research takeaway (2-3 câu — nên build gì / không build gì):**

```text
Nên build một workflow QA nhỏ: đọc track hiện có, dùng rule chuyển động/IoU làm baseline, thêm appearance/ReID để chấm điểm và xếp hạng các cửa sổ nghi ID switch, rồi mở đúng đoạn cho người sửa trong công cụ annotation. Không nên build annotation UI mới, không tự động đổi ID trên toàn video và chưa cần Agent tự lập kế hoạch/gọi nhiều tool. TrackEval chỉ dùng để đo trên subset có ground truth; nó không thay thế bước human review.
```
